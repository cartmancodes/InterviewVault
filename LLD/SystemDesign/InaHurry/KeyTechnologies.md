...
![Overall Structure](assets/cs5Ql6vMtghv.34lajmr238kgt.svg)
...
2. **Indexes:** [Indexes](https://www.hellointerview.com/learn/system-design/in-a-hurry/core-concepts#database-indexing) are a way of storing data in a way that makes it faster to query. For example, if you have a `users` table with a `name` column, you might create an index on the `name` column. This would allow you to query for users by name much faster than if you didn't have an index. Indexes are often implemented using a [B-Tree](https://en.wikipedia.org/wiki/B-tree) or a [Hash Table](https://en.wikipedia.org/wiki/Hash_table). The great thing about relational databases is (a) their support for arbitrarily many indexes, which allows you to optimize for different queries and (b) their support for multi-column and specialized indexes (e.g. geospatial indexes, full-text indexes).
...
![NoSQL Databases](assets/2xOVQTAYqY2W.363m0fmtkkojq.svg)
...
![Basic Blob Storage Example](assets/cVuVk-xeL2KH.1tawy_zcjb49n.svg)
...
![API Gateway](assets/pziKsE6PvMfR.3ikmaqblpu2cl.svg)
...
![A common setup for a horizontally scaled, authenticated service.](assets/C9AHQIRWV7ba.0rx49_m5hprw7.svg)
...
![Queue Buffer](assets/P6caMBd1Ah8S.0r3bfgz5lc6up.svg)
