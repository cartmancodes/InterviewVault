...
![Core Entities](assets/UqWfiTe_Sdv5.0dc_3xl71lpz2.svg)
...
![Simple Post Creation](assets/GSvCEXs3aSaa.3tj-1wx5c0wsk.svg)
...
![Following Service](assets/T50TalHjNI6j.03bjkru43jb9z.svg)
...
![Naive Feed Service](assets/JnvQzzz5V0Jr.2-p01f36ten8s.svg)
...
> In a real interview, I'd explain this to the interviewer exactly like that "I know this is not going to scale but I'm going to start with a naive solution and then solve the scaling problems separately". A very common failure mode for candidates we work with is that they get lost in the weeds of a particular scaling problem before having a (mostly) complete design. By covering the breadth of our requirements before we go into depth we can avoid this situation - as long as we keep moving quickly!
...
> People frequently wonder "what about if users page back 100 pages into {their feed, search results, etc}?" While this is a fair question, most systems will simply just not support this (try to go a few dozen pages deep into Google) because real users aren't doing this. At the end of the day, this fixed 200 number here could be increased or decreased by how much it impacts those tail users and the relative tradeoff in cost. We can always fall back to our naive solution of querying the base Follow and Post tables if we need to get more content for users who have reached the end of their feed.

![Adding a Feed Table](assets/AFCdTqxylRfd.3u7ar6_gra04_.svg)
...
![Async Workers for Post Creates](assets/_IOUCHcjJT2r.04t-h2u_e4n7n.svg)
...
![Async Workers for Post Creates](assets/zgd45nfdWhjT.1ehq7t00yyah6.svg)
...
![Distributed Post Cache](assets/Wp8y4I3a0LZl.2sk_xgrw-jfn-.svg)
...
![Hot key issue](assets/I3pRXtpMO9wN.0q_wkbcdq8ms9.svg)
...
![Redundant Post Cache](assets/wgNaIlU3icyI.36q1_kbm6rorw.svg)
