...
> **Overview**: LeetCode is a platform that helps software engineers prepare for coding interviews, offering a large catalog of problems, in-browser code submission with instant feedback, and periodic competitions with live leaderboards. The design's central challenge is running untrusted user code securely and in isolation while returning results within 5 seconds. Notably, this is a relatively small-scale system (a few hundred thousand users, ~4,000 problems), which keeps the architecture deliberately simple.
...
Imagine a coding gym with thousands of practice puzzles on the wall. You pick a puzzle, scribble your answer on a card, and drop it into a slot. Behind the wall, a strict, sealed booth takes your card, tries your answer against a checklist of examples, and slides back a "pass" or "fail" — all without ever letting your scribbles touch the gym's own equipment. The booth is sealed for a reason: some visitors might write mischievous answers, so the gym never runs them in the open. During a timed contest, a scoreboard by the door shows who has solved the most puzzles fastest, refreshing every few seconds so everyone can watch the race.

That sealed booth is a **sandboxed container** running your code, the checklist is the **test cases**, and the ever-refreshing scoreboard is a **Redis sorted set** feeding the live leaderboard. Everything in this design comes back to one idea: safely run a stranger's code and tell them quickly whether it worked.

## 🎯 Understanding the Problem

> 🧑‍💻 What is LeetCode ? Let's be honest; LeetCode needs no introduction. You're probably spending many hours a day there right now as you prepare. But, for the uninitiated, LeetCode is a platform that helps software engineers prepare for coding interviews. It offers a vast collection of coding problems, ranging from easy to hard, and provides a platform for users to answer questions and get feedback on their solutions. They also run periodic coding competitions.
...
![Requirements](assets/2Aq6FSI8349Z.291tl40-sqwy0.svg)
...
![Entities](assets/udPYGa34gBxR.3-7gjcwfy0ya-.svg)
...
![Viewing a list of problems](assets/stwBi0fERNta.3p5jwsw4tcl28.svg)
...
![Viewing a specific problem](assets/68i-mQQDiaTF.2j9sm_t5_lfkh.svg)
...
![Running user code in vm](assets/fjHnf8jeg4iK.3lnt91db3wbph.svg)
...
![Running user code in containers](assets/qRrRyO6z4osu.2tfbu65lyw9c0.svg)
...
![Running user code in containers](assets/asw8K-Vx3BSW.3k_s25xlk971v.svg)
...
![Leaderboard](assets/CZFnXVSfG5TR.0ub8sc28254jr.svg)
...
![Security](assets/8HXP4N6Pk6sk.3z6vltn50_jiw.svg)
...
![Redis Polling](assets/B1tH6O51nGgA.3fvte7hg0dfsl.svg)
...
Once the queue makes submission processing asynchronous, each submission moves through a clear lifecycle that the client tracks by polling:

```mermaid
stateDiagram-v2
    [*] --> Queued: user submits code<br/>API enqueues job
    Queued --> Processing: worker pulls from queue<br/>runs code in container
    Processing --> Completed: results written to DB + cache
    Processing --> Queued: container failure<br/>requeue and retry
    Completed --> [*]

    note right of Queued
        Client polls GET /check/{id} every 1s
        DB lookup returns "processing" until Completed
    end note
```
...
![LeetCode Network Tab](assets/leetcode-network.2xddnucvgeub6.png)
...
![Queue](assets/cRvwjVJP6dbT.2htznjud_079d.svg)
...
![Final](assets/MdHgyvKrUoea.1djqgm5_9umrf.svg)
...
- **Small scale is the defining constraint.** With only a few hundred thousand users and ~4,000 problems, a simple client-server (even monolithic) architecture is appropriate — resist over-engineering into microservices.
- **Running untrusted code safely is the core problem.** Never run user code in the API server. Prefer containers (lightweight, fast start, reusable per language) over VMs or serverless, and harden them with a read-only filesystem, CPU/memory bounds, an explicit timeout, no network access, and seccomp.
- **The 5-second timeout does double duty** — it protects against infinite loops and directly satisfies the non-functional requirement of returning results within 5 seconds.
- **Code execution is CPU-bound, so scale it horizontally.** Vertical scaling can't keep up with a competition's burst of submissions × ~100 test cases each; use auto-scaling container groups, optionally with a queue to buffer spikes and enable retries — at the cost of making submissions async (client polls `GET /check/:id`).
- **Redis sorted sets make the leaderboard cheap.** `ZADD` on each passing submission and `ZRANGE ... REV` for the top N replaces expensive per-request database queries; combined with ~5s client polling, this beats WebSockets for this modest scale.
- **Language-agnostic test cases** are achieved by serializing each input/output once (e.g. a tree as a level-order array) and giving every language a test harness that deserializes it — avoiding per-language test authoring.

## 📚 Related Concepts

- [Managing Long-Running Tasks](../Patterns/ManagingLongRunningTasks.md) — the pattern behind async submission processing with a queue and polling.
- [Real-Time Updates](../Patterns/Real-TimeUpdates.md) — polling vs. WebSockets trade-offs for the live leaderboard.
- [Redis](../DeepDives/Redis.md) — sorted sets and in-memory data structures powering the leaderboard.
- [Redis (Core Concepts)](../../CoreConcepts/Redis.md) — hands-on notes on Redis internals and commands.
- [Caching](../../CoreConcepts/Caching.md) — cache-the-leaderboard strategies and invalidation trade-offs.
- [DynamoDB](../DeepDives/Dynamodb.md) — the chosen NoSQL store, plus the GSI used to query submissions by `competitionId`.

---
*Source: [https://www.hellointerview.com/learn/system-design/problem-breakdowns/leetcode](https://www.hellointerview.com/learn/system-design/problem-breakdowns/leetcode)*
